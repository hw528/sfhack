...
def parse_args():
    parser = argparse.ArgumentParser(description='ASL Model Training with Webcam')
    ...
    parser.add_argument('--model_output', type=str, default='./asl_combined_model.joblib',
...
def collect_data(character, hand_mode='right', min_samples=30, camera_index=0, use_preprocessing=False):
    ...
    while collecting and cap.isOpened():
        ...
        cv2.putText(display_image, f"Collecting for: {character.upper()}", 
                   (20, 40), 
        ...
        # Show the image
        cv2.imshow(f'ASL Training - Character {character.upper()}', display_image)
...
def train_combined_model(data_dir, output_file):
    ...
    model = RandomForestClassifier(
        n_estimators=100, 
        max_depth=15,  # Increased for multi-class
        random_state=42
    )
